...
// Version 9.0-COMPLETE - ALLE v8.x FUNKTIONEN + JPY-PAAR OPTIMIZATION:
// 
// 🚀 NEUE v9.0 FEATURES:
...
// 
// 🚨 KRITISCHER JPY-FIX:
// Das Hauptproblem war: JPY-Paare verwendeten falsche Pip-Werte (0.001 statt 0.01)
// was zu 10x zu kleinen Lotsizes führte. v9.0 behebt dies vollständig.
//
...
bool ProcessSignal(string signal_id, string symbol_original, string direction, 
    ...
    double calc_entry = (final_entry > 0 && trade_type != ORDER_TYPE_BUY && trade_type != ORDER_TYPE_SELL) ? 
                        final_entry : entry;
    // v9.0 ENHANCED LOTSIZE CALCULATION WITH JPY OPTIMIZATION
    double lots = CalculateLots_v90_Enhanced(trading_symbol, calc_entry, sl, effective_risk_percent, trade_type, risk_message);
...
void ProcessPendingOrderSLTPUpdate(ulong ticket, string signal_id, double new_sl, double new_tp, int track_index) {
    ...
    if(sl_different) {
        ...
        if(trade.OrderDelete(ticket)) {
            ...
            // v9.0 ENHANCED LOTSIZE CALCULATION WITH JPY OPTIMIZATION
            double new_lots = CalculateLots_v90_Enhanced(symbol, order_price, new_sl, risk_percent, calc_order_type, risk_message);
...
//+------------------------------------------------------------------+
bool IsJPYPair_v90(string symbol) {
    string normalized = symbol;
...
bool ValidateLossPerLot_v90(string symbol, double loss_per_lot, double distance, string &validation_msg) {
    ...
    bool is_jpy = IsJPYPair_v90(symbol);
